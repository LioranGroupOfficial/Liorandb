...
This section is written from the real package source in
`C:\pro_projects\Liorandb-Rust\sdks\ldb-driver\ts-js\src`, not just from a
marketing README.

The examples on these driver pages are also aligned with the repository's
docs smoke test at
`C:\pro_projects\Liorandb-Rust\sdks\ldb-driver\ts-js\scripts\test-all.mjs`.
That script currently exercises:

- connection parsing and client lifecycle
- database and collection creation/drop flows
- CRUD, `countDocuments()`, indexes, `$text` queries, cursors, and aggregation
- auth, users, roles, cluster, settings, and backups admin services
